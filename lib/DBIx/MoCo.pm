...
__PACKAGE__->add_trigger(before_update => sub {
    my ($class, $self) = @_;
    defined $self or confess '$self is not specified'; 
...
sub cache {
    ...
    if (defined $v) {
        ...
        if ($v eq '') {
            if ($cache && $cache->can('remove')) {
                $cache->remove($k);
            }
            if ($session) {
                delete $session->{cache}->{$k} if $k;
            }
        } else {
            if ($class->cache_cols_only && ref($v) &&
                ...
                $cache->set($k,$o,$ex) if $cache;
                $session->{cache}->{$k} = $o if $session;
            } else {
                $cache->set($k,$v,$ex) if $cache;
                $session->{cache}->{$k} = $v if $session;

            }
        }
        return $v;
    } elsif ($k) {
        return ($session && $session->{cache}->{$k}) || ($cache && $cache->get($k));
    }
}
...
sub end_session {
    ...
    $session = undef;
    %DBIx::MoCo::DataBase::SQLSeen = ();
    %DBIx::MoCo::DataBase::SQLAndValuesSeen = ();
}
...
sub db_object {
    ...
    return $DB_OBJECT{$class} ||= $class->_db_object;
}
...
sub object_id {
    ...
    $PREFIX_CACHE{$class} ||= $class->object_id_prefix || '';
    my $prefix = $PREFIX_CACHE{$class};
    my ($key, $col);

    $PKEY_CACHE{$class} ||= $class->primary_keys;
    my $pkeys = $PKEY_CACHE{$class};

    $KEY_CACHE{$class} ||= $class->retrieve_keys || $pkeys;
    my @keys = @{$KEY_CACHE{$class}};

    if (defined $self and @keys) {
        my $false;
        $COLS_CACHE{$class} ||= $class->columns;
        my $cols = $COLS_CACHE{$class} or $false = 1;
        my $has_muid;
        for (@$cols) {
            if ($_ eq 'muid') {
                $has_muid = 1;
        ...
        if (not $false and $has_muid and @$pkeys == 1) {
            $key = $self->muid;
        } else {
            for (sort @keys) {
                if (not defined $self->{$_}) {
                    warn "$_ is undefined for $self";
                    return;
                }
                $key .= "-$_-" . $self->{$_};
            }
        ...
        my @args = @_;
        $col = defined $args[1] ? $args[0] : $pkeys->[0];
...
sub _merge_objects {
    ...
    my $i = 0;
    for (@$ordered) {
        $id2order{ $class->object_id( %$_ ) } = $i++;
    }

    my @merged;
    $#merged = $i + 1; ## malloc() を最小限に抑える

    for (@$cached, @$fromdb) {
        $merged[ $id2order{ $_->object_id } ] = $_;
    }

    my @res = grep { defined $_ } @merged;
    return \@res;
}
...
#     my $tied = tie my %idt, 'Tie::IxHash'; ## ordered Hash

#     for (@$ordered) {
#         $tied->Push($class->object_id( %$_ ) => undef);
#     }

#     for (@_) {
#         my $id = $_->object_id;

#         if (not exists $idt{$id}) {
#             die sprintf "assert (%s, %s)", $class, $id;
#         }
...
my %_db_to_driver_class;

sub _driver_class {
    my $class_or_self = shift;
    my $db = $class_or_self->db;
    return $_db_to_driver_class{$db} ||= do {
        my $vendor = $db->vendor;
        my $class = 'DBIx::MoCo::Driver::' . $vendor;
        $class->require or croak "Could not load driver $class: $@";
        $class;
    };
}

sub insert {
    my $driver = $_[0]->_driver_class;
    no strict 'refs';
    goto \&{"$driver\::insert"};
}
...
sub merge_with {
    ...
    for my $with_attr (@with_attrs) {
        ...
        my $with_records = scalar $rel->{class}->retrieve_multi(@queries);

        if (($class->icache_expiration || 0) > 0) {
            my %key_to_with = map { ( $_->$other_key => $_ ) } @$with_records;
            foreach (@$res) {
                $_->icache->{$with_attr} = $key_to_with{ $_->$my_key };
            }
        }
    }
...

{
    ...
    *DBIx::MoCo::can = sub {
        ...
        local $AUTOLOAD = $class . '::' . $method;
        local $@;
        eval { &DBIx::MoCo::AUTOLOAD($class) } unless *$AUTOLOAD{CODE};
...
sub _list {
    my $class = shift;
    $LIST_CLASS{$class} ||= $class->list_class;
    my $l = $LIST_CLASS{$class};
    if ($l) {
        if (not $LOADED_LIST{$l}) {
            $l->require;
...
Like C<create>, but allows detailed option. See also L<DBIx::MoCo::Driver::MySQL>.;

=item update

Like C<param>, but allows detailed option. See also L<DBIx::MoCo::Driver::MySQL>.;

=item replace

Executes C<REPLACE>, which is a MySQL extension to SQL. See also L<DBIx::MoCo::Driver::MySQL>.;
...
Naoya Ito, E<lt>platform@hatena.ne.jpE<gt>,
