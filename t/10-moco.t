...
sub object_id : Tests {
    ...
    is(Blog::Bookmark->object_id(user_id => 1, entry_id => 0), 'Blog::Bookmark-entry_id-0-user_id-1');
}
...
sub search : Tests {
    ...
    my $condition = ['name = :name', name => 'cinnamon'];
    ($u) = Blog::User->search(
        where => $condition,
    );
    ok $u;
    is $u->name, 'cinnamon';
    is_deeply $condition, ['name = :name', name => 'cinnamon'];
}
...
sub find : Tests {
    ...
    {
        ok (Blog::User->find('user_id'));
        ok !Blog::User->find(user_id => 0);
    }
}
...
sub has_many_limit_opts : Tests {
    my $user = Blog::User->retrieve(1);

    ok $user->bookmarks->size > 1;
    is $user->bookmarks(0, 1)->size, 1;

    Blog::User->has_many(
        bookmarks_limited => 'Blog::Bookmark',
        { key => 'user_id', limit => 1 },
    );

    is $user->bookmarks_limited->size, 1;
    is $user->bookmarks_limited(0, 0)->size, $user->bookmarks->size;
}
